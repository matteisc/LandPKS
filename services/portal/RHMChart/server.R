...
shinyServer(function(input, output,session) {
  ...
  output$PlotNames <- renderUI({
    ...
    #print(searchStr["CSV_URL"])
    validate(
      need("CSV_URL" %in% names(searchStr) ,paste0('No data to display!',searchStr)),
      need(searchStr["CSV_URL"] != "" ,'No data to display!')
    )
    
    prepareData(searchStr["CSV_URL"])
...
#   
#   observe({
#  #   updateCheckboxInput(session , "checkAll" , value = length(input$checkGroup) == length (plotNames))
#   })
  ...
  output$chart1 <- renderGvis({
    ...
    validate(need(input$checkGroup, 'Check at least one plot!' ))    
