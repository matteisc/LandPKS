...
def createPrivateFolder_DAT():
   PRIVATE_FOLDER_ACCESS_DAT = os.path.join("C:\\xampp\\htdocs\\APEX\\Python_APEX\\3_WISE_SOL_PROJECT\\Result_HWSD\\Private\\%s" % (str(ID)), "DAT")
   return PRIVATE_FOLDER_ACCESS_DAT 
def createPrivateFolder_SOL():
   PRIVATE_FOLDER_ACCESS_SOL = os.path.join("C:\\xampp\\htdocs\\APEX\\Python_APEX\\3_WISE_SOL_PROJECT\\Result_HWSD\\Private\\%s" % (str(ID)), "SOL")
...
def main():
   try:
       ...
       dat_folder = os.path.join("C:\\xampp\\htdocs\\APEX\\Python_APEX\\3_WISE_SOL_PROJECT\\Result_HWSD\\DATFiles\\",str(mu_global))
       sol_folder = os.path.join("C:\\xampp\\htdocs\\APEX\\Python_APEX\\3_WISE_SOL_PROJECT\\Result_HWSD\\SOLFiles\\",str(mu_global))
       ...
       if (os.path.exists(dat_folder) and os.path.exists(sol_folder)):
          ...
          os.system("python main_HWSD_ROSETTA.py -d \Database\HWSD.mdb -m %s -model %s" %(str(mu_global),ROSETTA_MODEL))
          copyFolder(dat_folder, PRIVATE_FOLDER_ACCESS_DAT) 
          copyFolder(sol_folder, PRIVATE_FOLDER_ACCESS_SOL)
   except Exception,err:
       sys.exit(err)
