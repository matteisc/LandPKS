...
def calculator_similarity_cosin_vector_space_model_MULTI_STEP(in_csv, out_csv,entry_plot_identify,entry_plot_data,data_type):
    try:
       f = open(in_csv, "rb")
       l = UnicodeReader(f)
       l.next()
       ...
       entry_plot_data_togo_landcover_vegind = [entry_plot_data[12],entry_plot_data[13],entry_plot_data[14],entry_plot_data[15],entry_plot_data[16],entry_plot_data[17],entry_plot_data[18]
                                                #entry_plot_data[19],entry_plot_data[20],entry_plot_data[21]
                                               ]
       entry_plot_data_clim_numerous = [entry_plot_data[2],entry_plot_data[3],entry_plot_data[4],
                                        #entry_plot_data[5], KOPGEIGER
                                        #entry_plot_data[6], LAO_LGP
                                        entry_plot_data[7],entry_plot_data[8],entry_plot_data[9],entry_plot_data[10],entry_plot_data[11]]
       
       entry_plot_data_togo_landcover_vegind_numerous = [entry_plot_data[12],
                                                         #entry_plot_data[13], GEOLAGE
                                                         entry_plot_data[14],
                                                         #entry_plot_data[15],LADNOFRM GLOBAK
                                                         entry_plot_data[16],entry_plot_data[17],entry_plot_data[18]
                                                #entry_plot_data[19],entry_plot_data[20],entry_plot_data[21]
                                               ]
       ...
       out.writerow("\n")
       out.writerow(['PLOT_ID','WISE3_ID','SIMILARITY_INDEX_CLIM','SIMILARITY_INDEX_TOGO_VEGIND_LANDCOVER',''])
       ...
       if (data_type == "1" or data_type == 1):
           ...
           for row in l:
               ...
               WISE_3_TOGO_LANDCOVER_VEGIND_DATA = [float(row[13]),float(row[14]),float(row[15]),float(row[16]),float(row[17]),float(row[18]),float(row[19])
                                                #float(row[20]),float(row[21]),float(row[22])
                                               ]
               WISE_3_CLIM_DATA_NUMEROUS = [float(row[3]),float(row[4]),float(row[5]),
                                        #float(row[6]), KOPGEIGER
                                        #float(row[7]), FAO_LGP
                                        float(row[8]),float(row[9]),float(row[10]),float(row[11]),float(row[12])]
               WISE_3_TOGO_LANDCOVER_VEGIND_DATA_NUMEROUS = [float(row[13]),
                                                         #float(row[14]), GEOLAGE
                                                         float(row[15]), 
                                                         #float(row[16]), LANDFORM_GLOBAL
                                                         float(row[17]),float(row[18]),float(row[19])
                                                #float(row[20]),float(row[21]),float(row[22])
                                               ]
               WISE_3_RECORD = [WISE_3_ID,WISE_3_CLIM_DATA,WISE_3_TOGO_LANDCOVER_VEGIND_DATA,WISE_3_CLIM_DATA_NUMEROUS,WISE_3_TOGO_LANDCOVER_VEGIND_DATA_NUMEROUS]
               WISE_3_LIST.append(WISE_3_RECORD)
       elif(data_type == "2" or data_type == 2):
           print "\n====Run Calculation based on NUMEROUS + CATEGORICAL Data Fields ============="
           print "\n====DECISION TREE Filtering : Selected Tranning Set RECOREDS same KOP-GEIGER Category with User Input==========="
           for row in l:
       ...
       for i in range(0,len(WISE_3_LIST)):
           try:
               ...
               WISE_3_CLIM_DATA_NUMEROUS = WISE_3_LIST[i][3]
               WISE_3_TOGO_LANDCOVER_VEGIND_DATA_NUMEROUS = WISE_3_LIST[i][4]
               cosin_angle_clim_data = angle(entry_plot_data_clim_numerous, WISE_3_CLIM_DATA_NUMEROUS) 
               #out.writerow([str(entry_plot_identify[0])+"-"+ str(entry_plot_identify[1]),str(WISE_3_ID),str(cosin_angle)])
               WISE_3_SIMILARITY_OBJECT = [str(entry_plot_identify[0])+"-"+ str(entry_plot_identify[1]),str(WISE_3_ID),str(cosin_angle_clim_data),WISE_3_CLIM_DATA,WISE_3_TOGO_LANDCOVER_VEGIND_DATA,WISE_3_TOGO_LANDCOVER_VEGIND_DATA_NUMEROUS]
       ...
       for i in range(0,len(WISE_3_SIMILARITY_RESULT_CLIM_SORTED_100)):
           try:
               ...
               WISE_3_TOGO_LANDCOVER_VEGIND_DATA_NUMEROUS = WISE_3_SIMILARITY_RESULT_CLIM_SORTED_100[i][5]
               cosin_angle_clim = WISE_3_SIMILARITY_RESULT_CLIM_SORTED_100[i][2]
               cosin_angle_togo_landcover_vegind_data = angle(entry_plot_data_togo_landcover_vegind_numerous, WISE_3_TOGO_LANDCOVER_VEGIND_DATA_NUMEROUS) 
               #out.writerow([str(entry_plot_identify[0])+"-"+ str(entry_plot_identify[1]),str(WISE_3_ID),str(cosin_angle)])
               ...
               print err
               pass
       print "Sorting TOGOG LANDCOVER VEGIND DATA.............."
...
def main():
    ...
    if (SIMILARITY_METHOD == 'cosine_vector_space_model'):
        ...
        result_multiple_step = calculator_similarity_cosin_vector_space_model_MULTI_STEP(WISE3_LANDPKS_INTPUT_CSV_STANDALIZATION_FILE,WISE3_LANDPKS_OUTPUT_CSV_SIMILARITY_INDEX_FILE,ENTRY_PLOT_IDENTIFY,ENTRY_PLOT_DATA,DATA_TYPE)
#################################################
####MAIN
#################################################
